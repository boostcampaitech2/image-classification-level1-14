...
class StratifiedMaskDataset(MaskBaseDataset):
    num_classes = 3 * 2 * 3
    ...
    def create_stratified_index(self, val_ratio):
        ...
        # for each case, split val_ratio and concat to build validation set with stratified ratio
        for c in df.classes.unique():
            class_idx = df[df.classes == c].index.tolist()
            np.random.shuffle(class_idx)

            split_idx = int(len(class_idx) * (1 - val_ratio))
            train_idx += class_idx[:split_idx]
